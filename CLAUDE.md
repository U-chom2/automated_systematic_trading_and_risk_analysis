...
5. 設計方針の遵守
   - **簡略化・簡素化の禁止**: 設計ドキュメント（方針.md等）に記載された仕様を勝手に簡略化しない
   - **アーキテクチャの維持**: 指定されたモデル（LSTM、Transformer、ModernBERT、PPO等）を勝手に変更しない
   - **機能の完全実装**: 「軽量版」「簡易版」「デモ版」への置き換えを行わない
   - **性能問題の報告**: PCの性能不足の場合は、簡略化せずにエラーとして報告する
   - **設計意図の尊重**: 複雑な実装には理由があるため、勝手に単純化しない
