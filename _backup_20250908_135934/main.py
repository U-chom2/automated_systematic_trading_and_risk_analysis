...
class AITradingSystem:
    ...
    def demo_mode(self) -> Dict[str, float]:
        ...
        # ダミーデータ生成（すべて0で明確にダミーとわかるように）
        logger.error("FALLBACK: Using all-zero dummy data for demonstration")
        
        # 日経225データ（30日分、すべて0）
        nikkei_data = np.zeros((30, 3))  # [high, low, close] all zeros
        
        # ターゲット株データ（30日分、すべて0）
        target_data = np.zeros((30, 3))  # [high, low, close] all zeros
